const RegexValidator = {
  RegexCSTValidator: (
    ...
    let regex = new RegExp(
      `${mt[0].leastOneLowerCaseletter ? `(?=.*[a-z])` : ""}^${
        mt[0].leastOneUpperCaseletter ? `(?=.*[A-Z])` : ""
      }${mt[0].leastOneletter ? `(?=.*[A-Za-z])` : ""}${
        mt[0].leastOneNumber ? `(?=.*\\d)` : ""
      }${
        mt[0].leastOneSpecialcharacter ? `(?=.*[@$!%*#?&])` : ""
      }[A-Za-z\\d@$!%*#?&]{${mt[0]?.min},${mt[0]?.max}}$`
    );
